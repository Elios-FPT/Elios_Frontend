...
const ManagePrompts = () => {
  ...
  const [groupedPrompts, setGroupedPrompts] = useState([]); // { name, latest, activeVersion, totalVersions, trafficSummary }
  ...
  const fetchPrompts = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(API_ENDPOINTS.LIST_PROMPTS, {
        params,
        ...
        res.data.prompts.forEach(p => {
          if (!grouped[p.prompt_name]) {
            grouped[p.prompt_name] = {
              name: p.prompt_name,
              latest: p, // latest version (by created_at)
              versions: [],
              activeVersion: null,
              totalVersions: 0,
              hasActive: false,
          ...
          if (p.is_active) {
            ...
            group.trafficSummary += p.traffic_percentage;
          }
        });
  ...
  const handleDelete = async (promptName) => {
    if (!window.confirm(`Soft delete prompt "${promptName}" and all its versions?`)) return;

    try {
      // Delete latest version → backend sẽ soft-delete toàn bộ (nếu thiết kế vậy)
        ...
        <div className="main-tabs-container">
          <div className="tab-header">
            ...
            <p className="intro-text">
              Manage prompt templates • A/B testing • Version control • Analytics
            </p>
          ...
          <div className="filters-bar" style={{ margin: 20 }}>
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <input
                  ...
                  onChange={e => setFilters(prev => ({ ...prev, include_deleted: e.target.checked, page: 1 }))}
                />
                <span style={{ color: 'var(--text-secondary)' }}>Show Deleted</span>
              </label>
            </div>

            {paging && (
              <div style={{ color: 'var(--text-muted)' }}>
                {paging.total} prompt{paging.total !== 1 ? 's' : ''} • {groupedPrompts.reduce((sum, g) => sum + g.totalVersions, 0)} total versions
              </div>
            )}
          </div>
              ...
              groupedPrompts.map(group => (
                <div key={group.name} className="project-card">
                  ...
                  <div className="card-body">
                    <p className="card-meta">
                      ...
                      {group.activeVersion ? (
                        <span className="difficulty-badge easy">
                          v{group.activeVersion.version} ({group.activeVersion.traffic_percentage}% traffic)
                        </span>
                      ) : (
                    ...
                    <p className="card-meta">
                      ...
                      {group.latest.deleted_at && (
                        <span style={{ color: 'var(--accent-red)', marginLeft: 8 }}>Deleted</span>
                      )}
                    </p>
                  </div>
                  ...
                  <div className="card-actions">
                    <button
                      onClick={() => navigate(`/manage-prompts/${group.name}`)}
                    ...
                    <button
                      onClick={() => navigate(`/manage-prompts/new-version/${group.name}`)}
                      className="btn-edit"
                    >
                      New Version
                    </button>
